...
public class CreateTooltipPrefab : MonoBehaviour
{
    [ContextMenu("Create Tooltip Prefab")]
    public void CreateTooltipPrefabInResources()
    {
#if UNITY_EDITOR
...
#else
        Debug.LogWarning("CreateTooltipPrefab can only be used in Unity Editor");
#endif
    }
...
#if UNITY_EDITOR
[CustomEditor(typeof(CreateTooltipPrefab))]
public class CreateTooltipPrefabEditor : Editor
{
    public override void OnInspectorGUI()
    {
        DrawDefaultInspector();
        
        CreateTooltipPrefab creator = (CreateTooltipPrefab)target;
        
        if (GUILayout.Button("Create Tooltip Prefab"))
        {
            creator.CreateTooltipPrefabInResources();
        }
        
        EditorGUILayout.HelpBox("Нажмите кнопку выше, чтобы создать префаб тултипа в папке Resources/UI/", MessageType.Info);
    }
}
#endif 